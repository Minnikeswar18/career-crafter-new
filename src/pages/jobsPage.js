...
function JobsPage() {
  ...
  const onLoad = async () => {
    axios.get(`${process.env.REACT_APP_BACKEND_URL}/job/myjobs`)
      .then((response) => {
  ...
  const addJob = async (event, handleClose) => {
    ...
    axios.post(`${process.env.REACT_APP_BACKEND_URL}/job/add`, {newJob})
      .then(async (response) => {
        await onLoad();
        handleClose();
        setShowLoader(false);
  ...
  const deleteJob = async (jobId) => {
    setShowLoader(true);
    axios.delete(`${process.env.REACT_APP_BACKEND_URL}/job/delete/${jobId}`)
      .then(async (response) => {
        $('#search-bar').val('');
        await onLoad();
        setShowLoader(false);
  ...
  return (
    <div className='home-page'>
      ...
      <h1 className='page-heading'>Jobs Posted by You</h1>
